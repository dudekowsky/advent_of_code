defmodule Grid do
  ...
  def fill_grid do
    map = for x <- 0..999, y <- 0..999, into: %{}, do: {{x,y}, false}
  ...
  def turn_on([a,b], [c,d]) do
    Agent.update(__MODULE__, fn map -> for x <- a..c, y <- b..d, into: map, do: {{x,y}, true} end)
  end

  def turn_off([a,b], [c,d]) do
    Agent.update(__MODULE__, fn map -> for x <- a..c, y <- b..d, into: map, do: {{x,y}, false} end)
  end

  def toggle([a,b], [c,d]) do
    Agent.update(__MODULE__, fn map -> for x <- a..c, y <- b..d, into: map, do: {{x,y}, !(Map.get(map, {x,y}))} end)
  end
