...
[_name, _, cap, _, dur, _, flav, _, text, _, cal] = line |> String.replace( [",",".",":"], "") |> String.strip |> String.split
val = [cap, dur, flav, text, cal] |> Enum.map(&String.to_integer(&1))
acc ++ [val]
end)
...
combinations = for ax <-0..100 do
  for bx <- 0..(100 - ax) do
    for cx <- 0..(100 - ax - bx) do
      for dx <- 0..(100 - ax - bx - cx), (ax + bx + cx + dx ) == 100 and
      (ax* a5 + bx *b5 + cx * c5 + dx * d5) == 500,
      do: [ax,bx,cx,dx]
    end
  end
end
